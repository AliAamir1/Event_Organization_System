...
cloudinary.config({
  cloud_name: "drvdi2fs3",
  api_key: "132674462222646",
  api_secret: "tYH88dSD9B-hvTFK6MOnCyCDcbg",
});
...
eventRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    try {
      ...
      const offDays = JSON.parse(req.body.offDays);
      const newEvent = new Event({
        ...
        location: location,
        advanceBookingDuration: req.body.advanceBookings,
        offDays: offDays,
        images: req.body.cloudinaryImages.split(","),
      });
      console.log(req.body.cloudinaryImages, "cloudinary images");
      console.log(req.body);

      const event = await newEvent.save();
      const vendor = await Vendor.findById(req.body.vendorId);
      console.log(event, "printing id");
...
eventRouter.put(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    try {
      ...
      Editevent.advanceBookingDuration = req.body.advanceBookings;
      Editevent.offDays = offDays;
      Editevent.images = req.body.cloudinaryImages.split(",");
...
// helper function to compare if two arrays match
...
eventRouter.put("/specialArrangement/:eventId", async (req, res) => {
  const event = await Event.findById(req.params.eventId);
  const arrangment = req.body.arrangement;
  if (event) {
    for (let i in event.specialArrangements) {
      if (arraysMatch(event.specialArrangements[i], arrangment)) {
        event.specialArrangements.splice(i, 1);
        await event.save();
        res.send({ message: "Arrangment deleted", event });
        return;
      }
    }
  }
...
eventRouter.post(
  ...
  expressAsyncHandler(async (req, res) => {
    ...
    const order = await Order.findOne({
      ...
      "orderItems.0._id": eventId,
    });

    if (!order) {
      return res.status(400).send({ message: "Bookings required for reviews" });
    }
    if (event) {
